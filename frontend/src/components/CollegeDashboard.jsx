...
const CollegeDashboard = () => {
  ...
  //       method: "POST",
  //       headers: { "Content-Type": "application/json" },
  //       body: JSON.stringify({ collegeId, seatMatrix: JSON.parse(seatMatrix) }),
  //     });
  //     const data = await res.json();
  //     alert("✅ Seat matrix submitted successfully",data);
  //     console.log(data);
  //   } catch (err) {
  ...
  const submitSeatMatrix = async () => {
    try {
      const response = await fetch("http://localhost:3001/api/college/submit-seats", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          collegeId,
          seatMatrix: typeof seatMatrix === "string" ? JSON.parse(seatMatrix) : seatMatrix,
        }),
      });
